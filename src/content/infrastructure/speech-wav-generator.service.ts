...
export class SpeechWavGeneratorService extends SpeechGeneratorService {
  ...
  constructor(private readonly speechAzureService: SpeechAzureService,
  ...
  async createAudioFromContent(content: Content): Promise<Audio> {
    ...
    const audioFilename = await this.convertTextToSpeech(plainTextDescription, content);
    ...
    const speechFileUrl = await this.storeSpeechFile(localFilePath, destinationFilePath);
  ...
  private extractSpeakableTextFromContent(content: Content): string {
    const plainTextDescription = this.textFormatter.htmlToText(content.description);
    return plainTextDescription;
  }


  private async convertTextToSpeech(plainText: string, content: Content): Promise<string> {
    const audioFilename = `${content.contentId}.wav`;
    await this.speechAzureService.fromTextToSpeech(plainText, audioFilename);
    return audioFilename;
  }

  private storeSpeechFile(filePath: string, destination: string): Promise<string> {
