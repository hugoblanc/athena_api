...
import type { PropositionLoi, Depute, Section, Article } from './types';
import { normalizeGroupePolitique } from './types';
import { DeputeScraper } from './depute-scraper';
...
export class PropositionScraper {
  ...
  constructor(private deputeScraper: DeputeScraper) {}
  ...
  async scrapeProposition(url: string): Promise<PropositionLoi | null> {
    try {
      ...
      const dateMiseEnLigne = dateDepot || new Date().toISOString();
